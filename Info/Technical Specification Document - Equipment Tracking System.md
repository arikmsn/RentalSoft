...
To support a global user base and specifically the primary Hebrew-speaking users, the system must be built with full internationalization capabilities from the ground up.

### 8.1 Language Support:
*   **Default Language:** Hebrew (עברית) must be the default language for all user-facing texts, labels, buttons, and messages.
*   **Secondary Language:** Full support for English as a secondary language.
*   **Language Switching:** The system should provide a clear and accessible mechanism for users to switch between Hebrew and English.

### 8.2 Text Externalization:
*   All user-facing strings, including static texts, button labels, error messages, and dynamic content, must be externalized into separate language files (e.g., JSON, YAML, or JavaScript objects).
*   This approach will facilitate easy translation, updates, and maintenance of language content without modifying the core codebase.

### 8.3 Right-to-Left (RTL) Support:
*   The entire user interface must fully support **Right-to-Left (RTL)** layout for Hebrew.
*   This includes text direction, alignment of UI elements (e.g., sidebar on the right, form fields alignment), table column order, and icon mirroring where appropriate.
*   The chosen UI framework (React with Tailwind CSS) should be configured to handle RTL styling dynamically based on the selected language.

### 8.4 Date and Number Formatting:
*   Dates, times, and numbers should be formatted according to the conventions of the selected language (e.g., DD/MM/YYYY for Hebrew, MM/DD/YYYY for English).

### 8.5 Implementation Recommendation:
*   Utilize a robust i18n library for React (e.g., `react-i18next` or `formatjs`) to manage translations and language switching.
*   Tailwind CSS can be extended with plugins or custom configurations to handle RTL layouts effectively.
